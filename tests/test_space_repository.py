...
def test_list_all_spaces(db_connection):
    ...
    assert repository.all() == [
        Space(1, 'Cottage', 'Wimborne', 'UK', 'A nice cottage', 27, 4),
...
def test_create_a_new_space(db_connection):
    db_connection.seed("seeds/MBnB.sql")
    repository = SpaceRepository(db_connection)
    repository.create(Space(None, 'Cabin', 'Tampaksiring', 'Indonesia', 'Located just a 20-minute scooter ride from the vibrant centre of Ubud, the Dome is a newly built one bedroom bamboo cabin situated within the Eco Six Resort.', 230, 3))
    assert repository.all() == [
        Space(1, 'Cottage', 'Wimborne', 'UK', 'A nice cottage', 27, 4),
        Space(2, 'Villa', 'Nice', 'France', 'A mediterranean villa with a sea view', 70, 2),
        Space(3, 'Alpine lodge', 'Brig-Glis', 'Switzerland', 'A cosy ski lodge with wood-burning fire', 95, 1),
...
def test_find_space_by_name_returns_single_space(db_connection):
    ...
    assert space == Space(3, 'Alpine lodge', 'Brig-Glis', 'Switzerland', 'A cosy ski lodge with wood-burning fire', 95, 1)